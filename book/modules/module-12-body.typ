...
What do you think is the answer? The answer seems to be yes to both, does it not? Well, pretty much.
But there are cases when the answer to either question can be no.

Since generally the equations we encounter in applications come from real life situations, it seems
logical that a solution always exists. It also has to be unique if we believe our universe is
deterministic. If the solution does not exist, or if it is not unique, we have probably not devised
the correct model. Hence, it is good to know when things go wrong and why.
...
#example(
  ...
  [
    Integrate to find the general solution $y = ln abs(x) + C$. The solution does not exist at
    $x = 0$. See the figure below with the slope field for this differential equation and some
    solutions.
      ...
      align(
        ...
        slope_field(
          ...
          yaxis: (
            position: 0,
            tip: tiptoe.stealth,
            filter: ((v, d) => calc.round(v, digits: 2) in (-3., -2., -1., 0, 1., 2., 3.)),
          ),
          xaxis: (
            position: 0,
            tip: tiptoe.stealth,
            filter: ((v, d) => calc.round(v, digits: 0) in (0, 1, 2, 3)),
          ),
          // xlabel: $x$,
          // ylabel: $y$,
...
It is hard to tell by staring at the slope field that the solution is not unique. Is there any hope?
Of course there is. We have the following theorem, known as Picard's theorem
...
#theorem(
  ...
  [
    If $f(x, y)$ is continuous (as a function of two variables) and $(partial f) / (partial y)$
    exists and is continuous near some $(x_0, y_0)$, then a solution to
...
Note that the problems $y' = 1 / x$, $y(0) = 0$ and $y' = 2 sqrt(abs(y))$, $y(0) = 0$ do not satisfy
the hypothesis of the theorem. Even if we can use the theorem, we ought to be careful about this
existence business. It is quite possible that the solution only exists for a short while.
