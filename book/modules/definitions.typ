...
#let show_def(definition_env: (..x) => x.pos().join(), name) = {
  ...
  let all_definitions = (
    model: definition_env(
      ...
      [
        ...
        + where the complexity of the world has been abstracted away to numbers, quantities, and
          their relationships.
    ...
    orderdiffeq: definition_env(
      ...
      [
        A differential equation is said to be _order $n$_ if the highest derivative that appears in
        the differential equation is the $n^"th"$ derivative.
      ],
    ),
    ...
    model_fit: definition_env(
      ...
      label: <def_model_fit>,
      [
        Given a model $M$ with parameters $k_1$, $k_2$, #sym.dots and data $D$, fitting the model
        $M$ to the data $D$ is the process of finding values for the parameters $k_1$, $k_2$,
        #sym.dots so that $M$ most accurately predicts the data $D$.
      ],
    ),
    slope_field: definition_env(
      ...
      [
        A _slope field_ for the differential equation $y' = f(x,y)$ is a collection of line segments
        that are tangent to different solutions of the differential equation.

        The line segments are generated by evaluating $f(x,y)$ over a rectangular grid of points,
        and at each point $(x,y)$ of the grid, creating a line segment of slope $f(x,y)$. The length
    ...
    equilibrium_solution: definition_env(
      ...
      [
        An _equilibrium solution_ to a differential equation or system of differential equations is
        a solution that is constant in the independent variable(s).
      ],
    ),
    equilibrium_classification_informal: definition_env(
      ...
      [
        ...
        - _repelling_ if there is a fixed distance so that locally, solutions tend away from $f$ by
          that fixed distance;
        - _stable_ if for any fixed distance, locally, solutions stay within that fixed distance of
          $f$; and,
    ...
    equilibrium_classification_formal: definition_env(
      ...
      [
        An equilibrium solution $f$ is called
        - _attracting at time $t_0$_ if there exists $epsilon > 0$ such that for all solutions $g$
          satisfying $abs(g(t_0) - f(t_0)) < epsilon$, we have
          $display(lim_(t -> infinity) f(t) = lim_(t -> infinity) g(t))$.
        - _repelling at time $t_0$_ if there exists $epsilon > 0$ and $delta > 0$ such that for all
          solutions $g$ that satisfy $0 < abs(g(t_0) - f(t_0)) < epsilon$ there exists $T in RR$ so
          that for all $t > T$ we have $abs(g(t) - f(t)) > delta$.
        - _stable at time $t_0$_ if for all $epsilon > 0$ there exists a $delta > 0$ such that for
          all solutions $g$ satisfying $abs(g(t_0) - f(t_0)) < delta$ we have
          $abs(g(t) - f(t)) < epsilon$ for all $t > t_0$.
        - _unstable at time $t_0$_ if $f$ is not stable at time $t_0$.

        $f$ is called attracting/repelling/stable/unstable if it has the corresponding property for
        all $t$.
      ],
    ),
    phase_portrait: definition_env(
      ...
      [
        A _phase portrait_ or _phase diagram_ is the plot of a vector field in phase space where
        each vector rooted at $(x, y)$ is tangent to a solution curve passing through $(x, y)$ and
        its length is given by the speed of a solution passing through $(x, y)$.
      ],
    ),
    linear_dependence_algebraic: definition_env(
      ...
      [
        The vectors $arrow(v)_1, arrow(v)_2, dots, arrow(v)_n$ are _linearly dependent_ if there is
        a non-trivial linear combination of $arrow(v)_1, dots, arrow(v)_n$ that equals the zero
        vector. Otherwise they are _linearly independent_.
      ],
    ),
    ...
    eigensolution: definition_env(
      ...
      [
        Let $M$ be an $n times n$ matrix and let $arrow(v)$ be an eigenvector for $M$ with
        associated eigenvalue $lambda$. The function
        $
    ...
    solution: definition_env(
      ...
      label: <def_solution>,
      [
        A _solution_ to a differential equation on a domain $D$ is a function with domain $D$ which
        satisfies the differential equation on $D$. (The domain $D$ is usually taken to be a
        connected set like an interval.)
      ],
    ),
    matrix_form: definition_env(
      ...
      [
        ...
        $
          ...
          dots.v\
        $
        is written in _matrix form_ if it is expressed as
        $
    ...
    subspace: definition_env(
      ...
      label: <subspace>,
      [
        A non-empty subset $V subset.eq RR^n$ is called a *subspace* if for all
        $arrow(u), arrow(v) in V$ and all scalars $k$ we have
    ...
    basis: definition_env(
      ...
      label: <basis>,
      [
        A basis for a subspace $cal(V)$ is a linearly independent set of vectors, $cal(B)$, so that
        $"span" cal(B)= cal(V)$.
      ],
    ),
...
#let show_def = show_def.with(definition_env: definition)
