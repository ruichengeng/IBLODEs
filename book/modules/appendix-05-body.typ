...
However, if we already know one solution to a differential equation, we can use that solution to
...
#example(
  ...
  [
    ...
    In other words, $y_1 v'' + (2 y'_1 + p(x) y_1) v' = 0$. Using $w = v'$, we have the first order
    linear equation
    $ y_1 w' + (2 y'_1 + p(x) y_1) w = 0. $

    To find the solution $y_2(x)$, we only need to solve this differential equation, which is a
    first-order differential equation in $w$.

    After solving this equation for $w$ (using integrating factor or separation of variables), we
    find $v$ by anti-differentiating $w$. We then form $y_2$ by computing $y_1 v$.

    Since we have a formula for the solution to the first order linear equation, we can write a
    formula for $y_2$:
    $
...
y_1(x) v(x)$ and find $v(x)$ as we did above. Also, the technique works for higher order equations
too: You get to reduce the order for each solution you find. So it is better to remember how to do
