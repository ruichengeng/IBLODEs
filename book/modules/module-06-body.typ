...
- How to find explicit solutions to systems of differential equations with constant coefficients by
  using the eigenvalues and eigenvectors of the coefficient matrix.
- How the real, distinct eigenvalues of a system of differential equations with constant
...
Starting with this module, we will be using several results from linear algebra. We will assume that
...
For equations like $y'=7 y$, we were lucky enough to guess-and-check our way to solutions
$y(t)=A e^(7 t)$ where $A in RR$ is a parameter. It turns out, by using insights from linear
algebra, we will be able to guess-and-check our way to solutions to certain systems of differential
equations as well.
...
$
where $A,B, k in RR$ are parameters. This implies
$
...
In the case of the single-variable equation $y'=7y$, multiplying a solution by a constant gave
...
The preceding example can be generalized to show that if you have a solution to a matrix
differential equation, then all scalar multiples of that solution are also solutions. This is in
turn a special case of a more general theorem.

#theorem(
  title: [Solutions Form a Subspace],
  [
    Let $arrow(r)'=M arrow(r)$ be a matrix differential equation and let $cal(S)$ be the set of all
    solutions. Then, $cal(S)$ is a subspace. In particular, $cal(S)$ is non-empty and closed under
    linear combinations.
  ],
)

_Proof:_ Suppose that $cal(S)$ is the set of all solutions to $arrow(r)'=M arrow(r)$ (where $M$ is a
square matrix). First, notice that
$
  arrow(0)' = arrow(0) = M arrow(0),
$
so $arrow(0) in cal(S)$. Next, suppose that $arrow(s)_1$ and $arrow(s)_2$ are solutions in $cal(S)$.
By definition, that means $arrow(s)'_1 = M arrow(s)_1$ and $arrow(s)'_2 = M arrow(s)_2$. By the
linearity of the derivative and of matrix multiplication, we may now compute
$
...
$
and so $alpha space.thin arrow(s)_1 + beta space.thin arrow(s)_2$ is also a solution in $cal(S)$.
...
The space $cal(F)^n$ is _large_. In fact, it is _infinite dimensional_, but the theorems of linear
algebra still apply (provided the proper definitions).

#show_def("linear_dependence_solutions")

#example(
  prompt: [Let $f(x)=x^2$, $g(x)=x$, and $h(x)=2x^2-x$. Show that ${f,g} subset.eq cal(F)^1$ is
    linearly independent but ${f,g,h} subset.eq cal(F)^1$ is linearly dependent],
  [
    ...
    $
    From the first equation, we see that $alpha = -beta$ and from the second that
    $alpha = -1 / 2 beta$. The only way for both of these conditions to be satisfied is if
    $alpha = 0$ and $beta = 0$. Thus ${f,g}$ is linearly independent.
    ...
    $
    which, by inspection, has a non-trivial solution $alpha=-2$, $beta=1$, and $gamma=1$. Thus
    ${f,g,h}$ is linearly dependent.
  ],
)

A historical technique for determining whether a set of functions is linearly independent is to
compute the determinant of the _Wronskian matrix_;#footnote[See
  https://en.wikipedia.org/wiki/Wronskian] while appeal to the Wronskian is occasionally useful, we
will stick with direct approaches in this text.

With the definition of linear independence/dependence pinned down, we can apply the usual Linear
Algebra definitions of subspace, basis, and dimension to $cal(F)^n$.
...
#theorem(
  ...
  [
    Consider the matrix differential equation $arrow(r)'(t) = M arrow(r)(t) + arrow(p)$ (or the
    single differential equation $y' = a y + b$). Every initial value problem for this differential
    equation has a unique solution. Further, the domain of every solution is $RR$.
  ],
)

For matrix differential equations, we will take $arrow(p)=arrow(0)$ in the above theorem.

Now, suppose that $M$ is an $n times n$ matrix. Let $cal(S)$ be the set of all solutions to
$arrow(r)' = M arrow(r)$. The above theorem states that every initial value problem
$
  arrow(r)' = M arrow(r) wide "and" wide arrow(r)(t_0) = mat(x_1; dots.v; x_n)
$
has a unique solution. Now observe that every solution must pass through _some point_ in $RR^n$ at
_some time_. Thus, the space of all solutions can be no "bigger" than the set of all initial
conditions. Notice that we have $n+1$ parameters to choose for initial conditions. We have $n$
choices coming from $x_1$, ..., $x_n$ with one more choice coming from the value of $t_0$.

This means that the dimension of $cal(S)$ is bounded above by $n+1$.#footnote[To make this argument
  fully rigorous, we need to establish the existence of a smooth map between the space of initial
  conditions and the space of solutions and then invoke the theorem that smooth maps cannot increase
  the dimension of a space.] But, we can do slightly better.

#theorem(
  title: [Solutions pass through $t=0$],
  [
    Let $arrow(w)$ be a solution to the initial value problem $arrow(r)' = M arrow(r)$ with initial
    condition $arrow(r)(t_0) = arrow(r)_0$. Then, $arrow(w)$ is _also_ the solution to the initial
    value problem $arrow(r)'=M arrow(r)$ with initial condition $arrow(r)(0) = arrow(w)_0$ where
    $arrow(w)_0 = arrow(w)(0)$.
  ],
)
_Proof:_ Let $arrow(w)$ be a solution to the initial value problem $arrow(r)' = M arrow(r)$ with
initial condition $arrow(r)(t_0) = arrow(r)_0$. Based on the Existence & Uniqueness Theorem 1, we
know $arrow(w)_0=arrow(w)(0)$ is defined. Further, since there is a solution to every initial value
problem, $arrow(r)' = M arrow(r)$ with $arrow(r)(0)=arrow(w)_0$ has a solution. Finally, because
every solution passing through a set of initial conditions is unique, since $arrow(w)(t)$ already
passes through $arrow(w)_0$, we must have that $arrow(w)$ is the unique solution to the initial
value problem $arrow(r)' = M arrow(r)$ with initial condition $arrow(r)(0) = arrow(w)_0$.
$square.filled$

The preceding theorem shrinks the space of solutions. Originally we knew that the set of all
solutions can be obtained by solving initial value problems for all choices of $x_1$, ..., $x_n$,
*and* $t_0$. But now we know that fixing $t_0=0$ gives us the same solutions. Thus, by making $n$
choices, we can determine a solution. This means
$
  dim(cal(S)) <= n.
$
...
Recall that $M$ is an $n times n$ matrix and that $cal(S)$ is the set of all solutions to
$arrow(r)' = M arrow(r)$. We have established $dim(cal(S)) <= n$. We will now try to find the exact
dimension and a basis for $cal(S)$.

Before reading this part of the module, work through the Core Exercises
@ex:basic_system[]--@ex:system_eigen_solutions[].

Suppose that $arrow(v)$ is an eigenvector for $M$ with eigenvalue $lambda$ and consider
$arrow(s)(t)=arrow(v)e^(lambda t)$. Multiplying by $M$ we get
$
...
$
In other words, $arrow(s)' =lambda arrow(s)= M arrow(s)$, and so $arrow(s)$ is a solution to our
differential equation. This means that whenever we have an eigenvector/eigenvalue for $M$, we can
write down an explicit solution to $arrow(r)'=M arrow(r)$. Solutions constructed this way are called
_eigen solutions_.

#show_def("eigensolution")

We just proved that any eigen solution to $arrow(r)'=M arrow(r)$ is, indeed, a solution. Since eigen
solutions corresponding to linearly independent eigenvectors are linearly independent (justify this
fact!), we know
$
...
#theorem(
  ...
  [
    Let $M$ be an $n times n$ matrix and let $cal(S)$ be the set of all solutions to
    $arrow(r)'(t) = M arrow(r)(t)$. Then, if $M$ is diagonalizable,
    $
      dim(cal(S)) = n.
    $
  ],
...
The preceding section shows us that if we have a full set of eigenvectors#footnote["full set" means
  that for an $n times n$ matrix we have $n$ linearly independent eigenvectors.] for a matrix $M$,
_every solution_ can be expressed as a linear combination of the eigen solutions.
...
The above figure shows an eigen solution in phase space (dashed) along with two non-eigen solutions
(solid).

The converse also holds: solutions whose graphs in phase space are straight lines heading towards or
...
Now that we have a method to find the solutions to a matrix differential equation, we can study the
question:
#quote(block: true)[
  As time goes to infinity, what is the long-term behaviour of solutions? Do they tend towards or
  away from the equilibrium solution(s)?
]
...
$arrow(v)_1$
and $arrow(v)_2$. The general solution to the equation $arrow(r)'= M arrow(r)$ is
$
  arrow(r)(t) = c_1 arrow(v)_1 e^(lambda_1 t) + c_2 arrow(v)_2 e^(lambda_2 t),
$
where $c_1$ and $c_2$ parameters. We want to study the long-term behaviour of solutions, which
depends on the long-term behaviour of the exponential function:
$
...
We immediately see:
- If $lambda_1 < 0$ and $lambda_2 < 0$, then $lim_(t arrow infinity) arrow(r)(t) = arrow(0)$ no
  matter the values of $c_1$ and $c_2$.
- If $lambda_1 > 0$ and $lambda_2 > 0$, then $lim_(t arrow infinity) arrow(r)(t)$ diverges as long
  as at least one of $c_1$ and $c_2$ are non-zero.
...
Now consider the equilibrium solution $arrow(s)(t)=arrow(0)$.#footnote[There may be other
  equilibrium solutions, but this equilibrium solution always exists.] If
$arrow(r)(t) = c_1 arrow(v)_1 e^(lambda_1 t) + c_2 arrow(v)_2 e^(lambda_2 t)$ is "close to"
$arrow(s)(t)=arrow(0)$ at time $t=0$, then $c_1$ and $c_2$ are close to zero. We can now consider
cases based on $lambda_1$ and $lambda_2$:

- If $lambda_1$ or $lambda_2$ is positive, then there will be solutions that diverge $==>$
  $arrow(s)$ is unstable.
- If $lambda_1$ and $lambda_2$ are negative, then _all_ solutions will converge zero $==>$
  $arrow(s)$ is stable and attracting.
- If one of $lambda_1$ or $lambda_2$ is zero, then there are constant solutions near $arrow(s)$
  $==>$ $arrow(s)$ is not attracting nor repelling.
  - If $lambda_1=0$ and $lambda_2 < 0$, then $arrow(s)$ will be stable, because solutions either
    converge to $arrow(0)$ or are constant.
  - If $lambda_1=0$ and $lambda_2 > 0$, then $arrow(s)$ will be unstable, because solutions either
    diverge to infinity or are constant.
...
#example(
  prompt: [Suppose $arrow(r)'=M arrow(r)$ has a general solution
    $ arrow(r)(t) = alpha mat(1; 1) e^(t) + beta mat(0; 1) e^(-t). $
    where $alpha$ and $beta$ are parameters. Prove that the equilibrium solution
    $arrow(s)(t)=arrow(0)$ is unstable and _not_ repelling.
  ],
  [
    ...
    $
    and notice that both $arrow(r)_a$ and $arrow(r)_b$ are solutions for any $a,b in RR$. By
    choosing $a>0$ very small, we can ensure that $arrow(r)_a (0)$ is as close to $arrow(0)$ as we
    like. However, no matter the choice of $a$ (as long as it's positive), we have
    $
    ...
    $
    and so $arrow(r)_b$ always heads towards the equilibrium solution $arrow(s)(t)=arrow(0)$. Thus,
    $arrow(s)(t)=arrow(0)$ cannot be a repelling equilibrium solution, because there is at least one
    solution which heads towards it.
  ],
)
